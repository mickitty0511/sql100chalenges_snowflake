drop table if exists sql100.public.sales_summary;
create table sql100.public.sales_summary as
    with receipt_tb as (
        select
            customer_id
...
select 
*
from sql100.public.sales_summary
unpivot (
    total_amount for gender_cd in (male_amt, female_amt, unknown_amt)
) as up (generation, gender_cd, total_amt)
order by generation
;
